 ...
 */
object Top10Category_v2_top10Session {
  def main(args: Array[String]): Unit = {
    ...
    val mapRDD: RDD[(String, Int)] = filterRDD.map(action => (action.click_category_id + "-" + action.session_id, 1))
    val reduceRDD: RDD[(String, Int)] = mapRDD.reduceByKey(_ + _)
    val transRDD: RDD[(String, (String, Int))] = reduceRDD.map(
      t => {
        val words: Array[String] = t._1.split("-")
        (words(0), (words(1), t._2))
      }
    )
    val groupRDD: RDD[(String, Iterable[(String, (String, Int))])] = transRDD.groupBy(_._1)
    groupRDD.mapValues(
      iter => {
